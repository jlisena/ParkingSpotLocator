...
while videoCapture.isOpened():
    ...
    if config['parking_detection']:
        for ind, park in enumerate(parking_data):
            ...
            status = np.std(roi_gray) < 28 and np.mean(roi_gray) > 45
    ...
    if write_to_db_counter % config['write_to_db_wait'] == 0:
        populateDB.child().update({"parkingLot2": spots})
    ...
    # video control to pause at current frame
        ...
        while True:
            ...
            cv2.imshow('Lot 37', frame_out)

            if key2 == ord('p'):
                break

    # start video feed
    new_resize = cv2.resize(frame_out, (1200, 400))                    # Resize image
    cv2.imshow('Lot 37', new_resize)
